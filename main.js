...
function testStrength() {
  ...
  else{  if (hasSpecial(password) && hasNum(password) && hasUppercase(password) && hasLowerCase(password) && password.length >= MIN_LENGTH && password.length < 16) {
        password_strength = "Good";
        document.getElementById('password-strength').className = "good";
        setProgressBar(80, "good");
    }
    else if (hasSpecial(password) && hasNum(password) && hasUppercase(password) && hasLowerCase(password) && password.length >= 16) {
        password_strength = "Excellent";
        document.getElementById('password-strength').className = "excellent";
        setProgressBar(100, "excellent");
    }
